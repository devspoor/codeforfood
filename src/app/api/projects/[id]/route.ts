...
export async function PATCH(
  ...
  try {
    ...
    const allowedFields = ["name", "description", "status", "hide_amounts", "hide_paid", "show_payment_history", "show_expenses", "tasks_board_public", "tasks_board_editable", "currency"];
    ...
    for (const key of allowedFields) {
      if (data[key] !== undefined) {
        ...
        if ((key === "hide_amounts" || key === "hide_paid" || key === "show_payment_history" || key === "show_expenses" || key === "tasks_board_public" || key === "tasks_board_editable") && typeof data[key] !== "boolean") {
